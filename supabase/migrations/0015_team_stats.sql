-- Estatísticas de equipe (F1). Identidade = nome da equipe (Racing Point ≠ Aston Martin, como nas estatísticas oficiais).

create index if not exists race_results_series_team_idx on public.race_results (series, team);

create or replace view public.team_career_totals with (security_invoker = true) as
with res as (
  select team,
...
-- Temporadas em que as duas equipes disputaram: posição e pontos de cada uma no mundial de construtores
create or replace function public.team_head_to_head(p_a text, p_b text)
returns table (season int, a_position int, b_position int, a_points numeric, b_points numeric, a_wins bigint, b_wins bigint)
language sql stable security invoker as $$
  select a.season, a.position, b.position, a.points, b.points,
         (select count(*) from driver_results r where r.season = a.season and r.team = p_a and r.position = 1),
         (select count(*) from driver_results r where r.season = a.season and r.team = p_b and r.position = 1)
  from constructor_standings a join constructor_standings b on b.series = a.series and b.season = a.season and b.team = p_b
  where a.series = 'f1' and a.team = p_a order by a.season
$$;
