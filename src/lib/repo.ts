...
import type { Answer, ConstructorStanding, OpenPool, PoolSummary, Driver, DriverStanding, Pool, Prediction, Question, Race, RaceResult, Result, Standing } from './types';
...
export const listDrivers = async (season: number) =>
  throwIf(await supabase.from('drivers').select('*').eq('season', season).order('team')) as Driver[];
...
export const importSeason = async (season: number) => {
  ...
  throwIf(await supabase.from('races').upsert(b.races, { onConflict: 'season,round' }));
...
export const getSyncInfo = async (season: number) =>
  (await supabase.from('f1_sync').select('*').eq('season', season).maybeSingle()).data as { last_round_with_results: number | null; synced_at: string } | null;
export const listDriverStandings = async (season: number) =>
  throwIf(await supabase.from('driver_standings').select('*').eq('season', season).order('position')) as DriverStanding[];
export const listConstructorStandings = async (season: number) =>
  throwIf(await supabase.from('constructor_standings').select('*').eq('season', season).order('position')) as ConstructorStanding[];
export const listRaceResults = async (season: number, round: number) =>
  throwIf(await supabase.from('race_results').select('*').eq('season', season).eq('round', round).order('position')) as RaceResult[];
