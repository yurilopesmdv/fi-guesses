...
export default function Races() {
  ...
  const { races, drivers, loading, importing, refresh } = useYear(year);
  const [podiums, setPodiums] = useState<Podium>({});
  const [refreshing, setRefreshing] = useState(false);

  const loadPodiums = useCallback(async () => {
    const { data } = await supabase.from('race_results').select('round,position,driver_code').eq('season', year).lte('position', 3).order('position');
    const m: Podium = {};
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: space(2), paddingBottom: insets.bottom + space(3) }}
      ...
      {loading ? (
        ...
        <>
          <Muted style={{ marginBottom: space(1) }}>{races.length} GPs · {Object.keys(podiums).length} disputados{year === SEASON ? ' · puxe pra atualizar' : ''}</Muted>
          {races.map((r, i) => {
